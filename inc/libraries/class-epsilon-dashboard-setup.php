...
class Epsilon_Dashboard_Setup {
	...
	public function add_admin_notice() {
		...
		if ( empty( $this->notice ) ) {
			...
			$this->notice .= '<h1>' . sprintf( esc_html__( 'Welcome to %1$s', 'taxi' ), $this->theme['theme-name'] ) . '</h1>';
			$this->notice .= '<p>';
			$this->notice .=
				sprintf( /* Translators: Notice */
					esc_html__( 'Welcome! Thank you for choosing %3$s! To fully take advantage of the best our theme can offer please make sure you visit our %1$swelcome page%2$s.', 'taxi' ),
			...
			$this->notice .= '<p><a href="' . esc_url( admin_url( '?page=epsilon-onboarding' ) ) . '" class="button button-primary button-hero" style="text-decoration: none;"> ' . sprintf( esc_html__( 'Get started with %1$s', 'taxi' ), $this->theme['theme-name'] ) . '</a></p>';
		}
	...
	public function get_steps() {
		return array(
			array(
				'id'       => 'landing',
				'title'    => __( 'Welcome to taxi', 'taxi' ),
				'content'  => array(
					'paragraphs' => array(
						__( ' This wizard will set up your theme, install plugins and import demo content. It is optional & should take less than a minute.', 'taxi' ),
					),
				),
				'progress' => __( 'Getting Started', 'taxi' ),
				'buttons'  => array(
					'next' => array(
						'action' => 'next',
						'label'  => __( 'Let\'s get started <span class="dashicons dashicons-arrow-right-alt2"></span>', 'taxi' ),
					),
				),
			),
			array(
				'id'       => 'plugins',
				'title'    => __( 'Install Recommended Plugins', 'taxi' ),
				'content'  => array(
					'paragraphs' => array(
						__( 'taxi integrates tightly with a few plugins that we recommend installing to get the full theme experience, as we\'ve intended it to be. This is an optional step, but we recommend installing them as we think these hand-picked plugins work really nice with taxi and help enhance the overall experience.', 'taxi' ),
					),
				),
				'progress' => __( 'Plugins', 'taxi' ),
				'buttons'  => array(
					'next' => array(
						'action' => 'next',
						'label'  => __( 'Next <span class="dashicons dashicons-arrow-right-alt2"></span>', 'taxi' ),
					),
				),
			),
			array(
				'id'       => 'enjoy',
				'title'    => __( 'Almost ready', 'taxi' ),
				'content'  => array(
					'paragraphs' => array(
						sprintf( __( 'Your new theme has been all set up. Enjoy your new theme by <a href="%s">Colorlib</a>.', 'taxi' ), esc_url( 'https://www.colorlib.com' ) ),
						$this->get_permission_content(),
					),
				),
				'progress' => __( 'Finished', 'taxi' ),
				'buttons'  => array(
					'next' => array(
						'action' => 'customizer',
						'label'  => __( 'Allow & Finish', 'taxi' ),
					),
				),
			),
		);
	...
	public function get_permission_content() {
		...
		$html .= '<a href="#hidden-permissions" id="hidden-permissions-toggle"> ' . __( 'What permissions are being granted', 'taxi' ) . ' <span class="dashicons dashicons-arrow-down"></span></a>';
		$html .= '<div id="hidden-permissions" >
			<ul>
				<li>
					<span class="dashicons dashicons-admin-users"></span>
					<span class="content">
						<strong>' . __( 'YOUR PROFILE OVERVIEW', 'taxi' ) . '</strong>
						<small>' . __( 'Name and email address', 'taxi' ) . '</small>		
					</span>
				</li>
				<li>
					<span class="dashicons dashicons-admin-settings"></span>
					<span class="content">
						<strong>' . __( 'YOUR SITE OVERVIEW', 'taxi' ) . '</strong>
						<small>' . __( 'Site URL, WP Version, PHP Version, plugins and themes', 'taxi' ) . '</small>		
					</span>
				</li>
				<li>
					<span class="dashicons dashicons-admin-plugins"></span>
					<span class="content">
						<strong>' . __( 'CURRENT PLUGIN EVENTS', 'taxi' ) . '</strong>
						<small>' . __( 'Activation, deactivation and uninstall', 'taxi' ) . '</small>		
					</span>
				</li>
			</ul>
			</div>
	...
	public function get_actions() {
		...
		return array(
			...
			array(
				...
				'actions'     => array(
					array(
						'label'   => Epsilon_Init_Notify_System::plugin_verifier( 'elementor', 'installed', 'Elementor' ) ? __( 'Activate Plugin', 'taxi' ) : __( 'Install Plugin', 'taxi' ),
						'type'    => 'handle-plugin',
			...
			array(
				...
				'actions'     => array(
					array(
						'label'   => Epsilon_Init_Notify_System::plugin_verifier( 'one-click-demo-import', 'installed', 'One Click Demo Import' ) ? __( 'Activate Plugin', 'taxi' ) : __( 'Install Plugin', 'taxi' ),
						'type'    => 'handle-plugin',
			...
			array(
				'id'          => 'taxi-check-demoimport',
				'title'       => __( 'To import demo data go to Appearance > Import Demo Data', 'taxi' ),
				'description' => __( 'Before import demo data make sure your install one click demo import plugin.', 'taxi' ),
				'plugin_slug' => '',
				'state'       => false,
				'check'       => !empty( get_option( 'taxi_demodata_import' ) ) ? true : false,
				'actions'     => array(),
			)
		);
	}
	...
	public function get_tabs( $setup = array() ) {
		...
		return array(
			array(
				...
				'title'   => esc_html__( 'Getting Started', 'taxi' ),
				'hidden'  => false,
				'type'    => 'info',
				'content' => array(
					array(
						'title'     => esc_html__( 'Step 1 - Implement recommended actions', 'taxi' ),
						'paragraph' => esc_html__( 'We compiled a list of steps for you, to take make sure the experience you will have using one of our products is very easy to follow.', 'taxi' ),
						'action'    => '<a href="' . esc_url( admin_url() . '?page=epsilon-onboarding' ) . '" class="button button-primary">' . __( 'Launch wizard', 'taxi' ) . '</a>',
					),
					array(
						'title'     => esc_html__( 'Step 2 - Check our documentation', 'taxi' ),
						'paragraph' => esc_html__( 'Even if you are a long-time WordPress user, we still believe you should give our documentation a very quick Read.', 'taxi' ),
						'action'    => '<a target="_blank" href="https://colorlib.com/">' . __( 'Full documentation', 'taxi' ) . '</a>',
					),
					array(
						'title'     => esc_html__( 'Step 3 - Customize everything', 'taxi' ),
						'paragraph' => esc_html__( 'Using the WordPress Customizer you can easily customize every aspect of the theme.', 'taxi' ),
						'action'    => '<a target="_blank" href="' . esc_url( admin_url() . 'customize.php' ) . '" class="button button-primary">' . esc_html__( 'Go to Customizer', 'taxi' ) . '</a>',
					)
				),
			),
			array(
				'id'      => 'epsilon-actions',
				'title'   => esc_html__( 'Actions', 'taxi' ),
				'type'    => 'actions',
			...
			array(
				'id'     => 'epsilon-plugins',
				'title'  => esc_html__( 'Recommended Plugins', 'taxi' ),

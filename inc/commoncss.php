...
// enqueue css
function taxi_common_custom_css() {

	wp_enqueue_style( 'taxi-common', get_template_directory_uri().'/assets/css/common.css' );

	$headerBg          = ! empty( get_header_image() ) ? 'url(' . esc_url( get_header_image() ) . ')' : '';

	// Header Top Pattern image
	$headerTopBg       	= taxi_opt( 'taxi_header_top_pattern_bg' );
	$headerTopbgImg 	= json_decode( $headerTopBg );
	$headerTopImgAttr 	= '';
	if( ! empty( $headerTopbgImg->url ) ) {
		$headerTopImgAttr = 'background:url( ' .esc_url( $headerTopbgImg->url ). ' );';
	}

	$headerTextColor   = esc_attr( taxi_opt( 'taxi_headertextcolor', '#fff' ) );
	$headerbgcolor     = esc_attr( taxi_opt( 'taxi_headerbgcolor' ) );
	$headerOverlay     = esc_attr( taxi_opt( 'taxi_headeroverlaycolor' ) );
	$stickynavbarbg    = esc_attr( taxi_opt( 'taxi_header_navbarsticky_bgColor' ) );
	$navbarBgColor     = esc_attr( taxi_opt( 'taxi_header_navbar_bgColor' ) );
	$navmenuColor 			= esc_attr( taxi_opt( 'taxi_header_navbar_menuColor' ) );
	$navmenuHovColor 		= esc_attr( taxi_opt( 'taxi_header_navbar_menuHovColor' ) );
	$stickynavmenuColor 	= esc_attr( taxi_opt( 'taxi_header_sticky_navbar_menuColor' ) );
	$stickynavmenuHovColor 	= esc_attr( taxi_opt( 'taxi_header_sticky_navbar_menuHovColor' ) );
	$foftext1     	   = esc_attr( taxi_opt( 'taxi_fof_textonecolor_settings' ) );
	$foftext2     	   = esc_attr( taxi_opt( 'taxi_fof_texttwocolor_settings' ) );
	$fofbgcolor        = esc_attr( taxi_opt( 'taxi_fof_bgcolor_settings' ) );
	
	//Footer Widget Background
	$footerbgImg       = taxi_opt( 'taxi_footer_bgimg_settings' );
	...
	$footerbgColor     = esc_attr( taxi_opt( 'taxi_footer_widget_bgColor_settings' ) );
	$footerTextColor   = esc_attr( taxi_opt( 'taxi_footer_widget_color_settings' ) );
	$anchorcolor 	   = esc_attr( taxi_opt( 'taxi_footer_widget_anchorcolor_settings' ) );
	$anchorhovcolor    = esc_attr( taxi_opt( 'taxi_footer_widget_anchorhovcolor_settings' ) );
	$widgettitlecolor  = esc_attr( taxi_opt( 'taxi_footer_widgettitlecolor_settings' ) );
	$themecolor  	   = esc_attr( taxi_opt( 'taxi_themecolor' ) );

	$footerbtomtextcolor 		= esc_attr( taxi_opt( 'taxi_footer_bottom_textcolor_settings' ) );
	$footerbtomanchorcolor 		= esc_attr( taxi_opt( 'taxi_footer_bottom_anchorcolor_settings' ) );
	$footerbtomanchorhovercolor = esc_attr( taxi_opt( 'taxi_footer_bottom_anchorhovercolor_settings' ) );
	...
	$customcss ="
			...
			.widget_taxi_recent_widget .popular-title,
			...
			.widget-wrap .widget_taxi_recent_widget .single-post-list .details h6:hover {
				color: {$themecolor};
			}
			.genric-btn.primary:hover,
			.genric-btn.primary-border,
	...
	wp_add_inline_style( 'taxi-common', $customcss );

}
add_action( 'wp_enqueue_scripts', 'taxi_common_custom_css', 50 );
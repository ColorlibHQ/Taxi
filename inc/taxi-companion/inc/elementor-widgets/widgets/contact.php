...
/**
 *
 * Taxi elementor about page section widget.
 *
 * @since 1.0
 */
class Taxi_Contact extends Widget_Base {
    ...
    protected function _register_controls() {
        ...
        $this->start_controls_section(
            'contact_map',
            [
                'label' => __( 'Map Settings', 'taxi' ),
            ]
        );
        $this->add_control(
            'map_switch',
            [
                'label' => __( 'Show Map', 'taxi' ),
        ...
        $this->add_control(
            'map_address',
            [
                'label'         => esc_html__( 'Address', 'taxi' ),
                'type'          => Controls_Manager::TEXT,
                'label_block'   => true,
                'default'       => esc_html__( 'Rabindra Sorobor Dhanmondi Lake Rd', 'taxi' ),
                'description'   => esc_html__( 'Before use map widget make sure you are set google map api key from customizer -> theme options -> general -> google map api key', 'taxi' )
            ]
        );
        $this->add_control(
            'map_lat',
            [
                'label'       => esc_html__( 'Latitude', 'taxi' ),
                'label_block' => true,
                'type'        => Controls_Manager::TEXT,
                'default'     => esc_html__( '23.745227', 'taxi' )
            ]
        );
        $this->add_control(
            'map_lng',
            [
                'label'       => esc_html__( 'Longitude', 'taxi' ),
                'label_block' => true,
                'type'        => Controls_Manager::TEXT,
                'default'     => esc_html__( '90.377155', 'taxi' )
            ]
        );
        ...
        $this->start_controls_section(
            'contact_info',
            [
                'label' => __( 'Contact Info', 'taxi' ),
            ]
        );

        $this->add_control(
            'info', [
                'label'         => __( 'Create Contact Info', 'taxi' ),
                ...
                'fields'  => [
                    [
                        ...
                        'label_block' => true,
                        'type'        => Controls_Manager::TEXT,
                        'default'     => esc_html__( 'Dhaka, Bangladesh', 'taxi' )
                    ],
                    [
                        'name'    => 'desc',
                        'label'   => __( 'Contact Descriptions', 'taxi' ),
                        'type'    => Controls_Manager::WYSIWYG,
                        'default' => esc_html__( 'Write something...', 'taxi' )
                    ],
                    [
                        'name'  => 'icon',
                        'label' => __( 'Icon', 'taxi' ),
        ...
        $this->start_controls_section(
            'contact_form',
            [
                'label' => __( 'Contact Form', 'taxi' ),
            ]
        );
        $this->add_control(
            'contact_formshortcode',
            [
                'label'     => esc_html__( 'Form Shortcode', 'taxi' ),
                'type'      => Controls_Manager::TEXT,
                'label_block' => true
            ]
        );
        ...
        $this->start_controls_section(
            'style_content_color', [
                'label' => __( 'Style Content Color', 'taxi' ),
        ...
        $this->add_control(
            'color_title', [
                'label'     => __( 'Address Title Color', 'taxi' ),
        ...
        $this->add_control(
            'color_desc', [
                'label'     => __( 'Address Description Color', 'taxi' ),
                'type'      => Controls_Manager::COLOR,
                'default'   => '#777',
                'selectors' => [
        ...
        $this->add_control(
            'color_fbtntextcolor', [
                'label'     => __( 'Form Button Label color', 'taxi' ),
                'type'      => Controls_Manager::COLOR,
                'default'   => '#222222',
                'selectors' => [
        ...
        $this->add_control(
            'color_btnhovlc', [
                'label'     => __( 'Form Button Hover Label color', 'taxi' ),
                'type'      => Controls_Manager::COLOR,
                'default'   => '#f9d700',
                'selectors' => [
    ...
    wp_enqueue_script('taxi-map-custom');
                    ...
                    ?>
                        <div class="single-contact-address d-flex flex-row">
                            ...
                            <div class="contact-details">
                                ...
                                //
                                if( ! empty( $info['desc'] ) ) {
                                    echo taxi_get_textareahtml_output( $info['desc'] );
                                }
                                ?>
                            </div>
                        </div>
